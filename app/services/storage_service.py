...
@dataclass(frozen=True)
class StoredPersonalEvidenceFile:
    original_name: str
    stored_name: str
    storage_path: str
    mime_type: str
...
@dataclass(frozen=True)
class StoredPersonalTrainingEvidenceFile:
    original_name: str
    stored_name: str
    storage_path: str
    mime_type: str
...
@dataclass(frozen=True)
class StoredPersonalAuthorizationEvidenceFile:
    original_name: str
    stored_name: str
    storage_path: str
    mime_type: str
...
def build_personal_evidence_filename(personal, calificacion, original_filename, sha256) -> str:
    safe_original = secure_filename(original_filename or "")
    if not safe_original or "." not in safe_original:
        raise DocumentStorageError("El archivo no tiene un nombre o extensión válidos.")
    extension = safe_original.rsplit(".", 1)[1].lower()
    hash_short = re.sub(r"[^a-fA-F0-9]", "", str(sha256 or ""))[:8].lower()
    if len(hash_short) != 8:
        raise DocumentStorageError("No se pudo generar la huella del archivo de evidencia.")
    personal_code = slugify_filename_part(getattr(personal, "codigo", None), max_length=60).upper()
    personal_code = personal_code or f"PERSONAL_{getattr(personal, 'id', 'SIN_ID')}"
    title = slugify_filename_part(getattr(calificacion, "titulo", None), max_length=80)
    original_stem = safe_original.rsplit(".", 1)[0]
    descriptive = title or slugify_filename_part(original_stem, max_length=80) or "evidencia"
    fixed_length = len(f"{personal_code}_{hash_short}.{extension}")
    available = max(1, MAX_STORED_FILENAME_LENGTH - fixed_length - 1)
    descriptive = descriptive[:available].rstrip("_") or "evidencia"
    return f"{personal_code}_{descriptive}_{hash_short}.{extension}"


def store_personal_evidence_file(file_storage, *, personal, calificacion) -> StoredPersonalEvidenceFile | None:
    if not file_storage or not file_storage.filename:
        return None
    ...
    profile = get_onlyoffice_profile_by_extension(extension)
    relative_directory = Path(
        f"empresa_{int(personal.empresa_id)}",
        f"personal_{int(personal.id)}",
        f"calificacion_{int(calificacion.id)}",
        "evidencias",
    )
    destination_directory = (_storage_root() / relative_directory).resolve()
    destination_directory.mkdir(parents=True, exist_ok=True)
    if os.path.commonpath([str(_storage_root()), str(destination_directory)]) != str(_storage_root()):
        raise DocumentStorageError("La ruta de almacenamiento de evidencia no es válida.")
    ...
    try:
        ...
        if profile and profile.extension == "xlsx":
            try:
                ...
                raise DocumentStorageError(str(exc)) from exc
        sha256 = digest.hexdigest()
        stored_name = build_personal_evidence_filename(personal, calificacion, safe_original_name, sha256)
        destination = (destination_directory / stored_name).resolve()
        if os.path.commonpath([str(_storage_root()), str(destination)]) != str(_storage_root()):
            raise DocumentStorageError("La ruta de evidencia no es válida.")
        if destination.exists():
            raise DocumentStorageError("Ya existe una evidencia con el mismo nombre y contenido.")
        temporary.replace(destination)
    except Exception:
        temporary.unlink(missing_ok=True)
        raise
    ...
    storage_path = (relative_directory / stored_name).as_posix()
    return StoredPersonalEvidenceFile(
        original_name=original_name,
        stored_name=stored_name,
        storage_path=storage_path,
        mime_type=mime_type,
...
def build_personal_training_evidence_filename(capacitacion, original_filename, sha256, participante=None) -> str:
    safe_original = secure_filename(original_filename or "")
    if not safe_original or "." not in safe_original:
        raise DocumentStorageError("El archivo no tiene un nombre o extensión válidos.")
    extension = safe_original.rsplit(".", 1)[1].lower()
    hash_short = re.sub(r"[^a-fA-F0-9]", "", str(sha256 or ""))[:8].lower()
    if len(hash_short) != 8:
        raise DocumentStorageError("No se pudo generar la huella del archivo de capacitación.")
    code = slugify_filename_part(getattr(capacitacion, "codigo", None), max_length=60).upper()
    code = code or f"CAPACITACION_{getattr(capacitacion, 'id', 'SIN_ID')}"
    participant_part = ""
    if participante and getattr(participante, "personal", None):
        participant_code = slugify_filename_part(getattr(participante.personal, "codigo", None), max_length=40).upper()
        participant_part = f"_{participant_code}" if participant_code else f"_PARTICIPANTE_{participante.id}"
    name = slugify_filename_part(getattr(capacitacion, "nombre", None), max_length=80)
    original_stem = safe_original.rsplit(".", 1)[0]
    descriptive = name or slugify_filename_part(original_stem, max_length=80) or "evidencia"
    fixed_length = len(f"{code}{participant_part}_{hash_short}.{extension}")
    available = max(1, MAX_STORED_FILENAME_LENGTH - fixed_length - 1)
    descriptive = descriptive[:available].rstrip("_") or "evidencia"
    return f"{code}{participant_part}_{descriptive}_{hash_short}.{extension}"


def store_personal_training_evidence_file(
    file_storage,
    *,
    capacitacion,
    participante=None,
) -> StoredPersonalTrainingEvidenceFile | None:
    if not file_storage or not file_storage.filename:
        return None
    ...
    profile = get_onlyoffice_profile_by_extension(extension)
    relative_parts = [
        f"empresa_{int(capacitacion.empresa_id)}",
        f"capacitacion_{int(capacitacion.id)}",
        "evidencias",
    ]
    if participante:
        relative_parts.append(f"participante_{int(participante.id)}")
    relative_directory = Path(*relative_parts)
    destination_directory = (_storage_root() / relative_directory).resolve()
    destination_directory.mkdir(parents=True, exist_ok=True)
    if os.path.commonpath([str(_storage_root()), str(destination_directory)]) != str(_storage_root()):
        raise DocumentStorageError("La ruta de almacenamiento de evidencia de capacitación no es válida.")
    ...
    try:
        ...
        if profile and profile.extension == "xlsx":
            try:
                ...
                raise DocumentStorageError(str(exc)) from exc
        sha256 = digest.hexdigest()
        stored_name = build_personal_training_evidence_filename(
            capacitacion,
            safe_original_name,
            sha256,
        ...
        destination = (destination_directory / stored_name).resolve()
        if os.path.commonpath([str(_storage_root()), str(destination)]) != str(_storage_root()):
            raise DocumentStorageError("La ruta de evidencia de capacitación no es válida.")
        if destination.exists():
            raise DocumentStorageError("Ya existe una evidencia de capacitación con el mismo nombre y contenido.")
        temporary.replace(destination)
    except Exception:
        temporary.unlink(missing_ok=True)
        raise
...
def build_personal_authorization_evidence_filename(autorizacion, original_filename, sha256) -> str:
    safe_original = secure_filename(original_filename or "")
    if not safe_original or "." not in safe_original:
        raise DocumentStorageError("El archivo no tiene un nombre o extensión válidos.")
    extension = safe_original.rsplit(".", 1)[1].lower()
    hash_short = re.sub(r"[^a-fA-F0-9]", "", str(sha256 or ""))[:8].lower()
    if len(hash_short) != 8:
        raise DocumentStorageError("No se pudo generar la huella del archivo de autorización.")
    personal = getattr(autorizacion, "personal", None)
    personal_code = slugify_filename_part(getattr(personal, "codigo", None), max_length=60).upper()
    personal_code = personal_code or f"PERSONAL_{getattr(autorizacion, 'personal_id', 'SIN_ID')}"
    activity = slugify_filename_part(getattr(autorizacion, "actividad", None), max_length=80)
    original_stem = safe_original.rsplit(".", 1)[0]
    descriptive = activity or slugify_filename_part(original_stem, max_length=80) or "autorizacion"
    fixed_length = len(f"{personal_code}_{hash_short}.{extension}")
    available = max(1, MAX_STORED_FILENAME_LENGTH - fixed_length - 1)
    descriptive = descriptive[:available].rstrip("_") or "autorizacion"
    return f"{personal_code}_{descriptive}_{hash_short}.{extension}"


def store_personal_authorization_evidence_file(
    file_storage,
    *,
    autorizacion,
) -> StoredPersonalAuthorizationEvidenceFile | None:
    if not file_storage or not file_storage.filename:
        return None
    ...
    profile = get_onlyoffice_profile_by_extension(extension)
    relative_directory = Path(
        f"empresa_{int(autorizacion.empresa_id)}",
        f"personal_{int(autorizacion.personal_id)}",
        f"autorizacion_tecnica_{int(autorizacion.id)}",
        "evidencias",
    )
    destination_directory = (_storage_root() / relative_directory).resolve()
    destination_directory.mkdir(parents=True, exist_ok=True)
    if os.path.commonpath([str(_storage_root()), str(destination_directory)]) != str(_storage_root()):
        raise DocumentStorageError("La ruta de almacenamiento de evidencia de autorización no es válida.")
    ...
    try:
        ...
        if profile and profile.extension == "xlsx":
            try:
                ...
                raise DocumentStorageError(str(exc)) from exc
        sha256 = digest.hexdigest()
        stored_name = build_personal_authorization_evidence_filename(autorizacion, safe_original_name, sha256)
        destination = (destination_directory / stored_name).resolve()
        if os.path.commonpath([str(_storage_root()), str(destination)]) != str(_storage_root()):
            raise DocumentStorageError("La ruta de evidencia de autorización no es válida.")
        if destination.exists():
            raise DocumentStorageError("Ya existe una evidencia de autorización con el mismo nombre y contenido.")
        temporary.replace(destination)
    except Exception:
        temporary.unlink(missing_ok=True)
        raise
    ...
    storage_path = (relative_directory / stored_name).as_posix()
    return StoredPersonalAuthorizationEvidenceFile(
        original_name=original_name,
        stored_name=stored_name,
        storage_path=storage_path,
        mime_type=mime_type,
