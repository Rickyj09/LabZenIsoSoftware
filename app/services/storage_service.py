...
def store_document_file(file_storage, *, empresa_id, documento_id, version) -> StoredDocumentFile | None:
    if not file_storage or not file_storage.filename:
        return None
    ...
    relative_directory = Path(
        ...
        f"documento_{int(documento_id)}",
        f"v{_safe_version(version)}",
    )
    ...
    temporary = destination.with_name(f".{stored_name}.tmp")
